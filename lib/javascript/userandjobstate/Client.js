...
function UserAndJobState(url, auth, auth_cb) {
    ...
    this.list_services = function (auth, _callback, _errorCallback) {
    return json_call_ajax("UserAndJobState.list_services",
        [auth], 1, _callback, _errorCallback);
};

    this.list_services_async = function (auth, _callback, _error_callback) {
        deprecationWarning();
        return json_call_ajax("UserAndJobState.list_services", [auth], 1, _callback, _error_callback);
    };
    ...
    this.start_job = function (job, token, status, desc, progress, _callback, _errorCallback) {
    return json_call_ajax("UserAndJobState.start_job",
        [job, token, status, desc, progress], 0, _callback, _errorCallback);
};

    this.start_job_async = function (job, token, status, desc, progress, _callback, _error_callback) {
        deprecationWarning();
        return json_call_ajax("UserAndJobState.start_job", [job, token, status, desc, progress], 0, _callback, _error_callback);
    };

    this.create_and_start_job = function (token, status, desc, progress, _callback, _errorCallback) {
    return json_call_ajax("UserAndJobState.create_and_start_job",
        [token, status, desc, progress], 1, _callback, _errorCallback);
};

    this.create_and_start_job_async = function (token, status, desc, progress, _callback, _error_callback) {
        deprecationWarning();
        return json_call_ajax("UserAndJobState.create_and_start_job", [token, status, desc, progress], 1, _callback, _error_callback);
    };

    this.update_job_progress = function (job, token, status, prog, _callback, _errorCallback) {
    return json_call_ajax("UserAndJobState.update_job_progress",
        [job, token, status, prog], 0, _callback, _errorCallback);
};

    this.update_job_progress_async = function (job, token, status, prog, _callback, _error_callback) {
        deprecationWarning();
        return json_call_ajax("UserAndJobState.update_job_progress", [job, token, status, prog], 0, _callback, _error_callback);
    };

    this.update_job = function (job, token, status, _callback, _errorCallback) {
    return json_call_ajax("UserAndJobState.update_job",
        [job, token, status], 0, _callback, _errorCallback);
};

    this.update_job_async = function (job, token, status, _callback, _error_callback) {
        deprecationWarning();
        return json_call_ajax("UserAndJobState.update_job", [job, token, status], 0, _callback, _error_callback);
    };
